...
export default function MobileNav() {
  ...
  const menuItems = [
    ...
    { icon: <Shield className="w-6 h-6" />, text: "Nega BM SECURITY ?", href: "#" },
  ...
  return (
    <div className="md:hidden">
      <Button variant="ghost" size="icon" className="text-emerald-400" onClick={() => setIsOpen(true)}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-gray-900 bg-opacity-90 z-50 backdrop-blur-sm"
          >
            <div className="flex flex-col h-full">
              <div className="flex justify-end p-4">
                <Button variant="ghost" size="icon" className="text-emerald-400" onClick={() => setIsOpen(false)}>
                  <X className="h-6 w-6" />
                </Button>
              </div>
